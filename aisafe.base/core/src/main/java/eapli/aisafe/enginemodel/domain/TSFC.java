...
public class TSFC implements ValueObject {
    ...
    /** Accepted TSFC units for aviation engine specifications. */
    public static final String[] VALID_UNITS = { "g/kN/s", "lb/(lbf.h)", "mg/N/s" };
    ...
    public TSFC(final double value, final String unit) {
        ...
        Invariants.ensure(Arrays.asList(VALID_UNITS).contains(trimmed),
                "TSFC unit must be one of: " + String.join(", ", VALID_UNITS));
        this.value = value;
        this.unit = trimmed;
    }
